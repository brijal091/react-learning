    ...
    return (
        <div>
            <h2 className="mt-5">{props.heading}</h2>
                ...
                <div className="mb-2">
                    <textarea className="form-control" value={text} id="textarea" rows="10" onChange={handleOnChange}></textarea>
                </div>        
                <div className="col-auto">
                    {/* We can use setText like the following */}
                    <button onClick={handleClick} className="btn btn-primary mb-3">Convert to Uppecase</button>
                </div>
        </div>
  )
}
